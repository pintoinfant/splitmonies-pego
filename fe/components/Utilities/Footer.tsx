...
export default function Footer() {
  return (
    <footer id="contact" className="bg-black/50">
      <div className="mx-auto max-w-7xl overflow-hidden px-6 py-20 sm:py-14 lg:px-8">
        <h1 className="text-4xl lg:text-4xl leading-[3rem] text-center text-white font-black">
          Made with ❤️ by your frens at
          <Image
            src="/logo/logo.png"
            className="-mt-3 h-8 w-8 lg:h-auto lg:w-[40px] ml-4 inline"
            width={464}
            height={500}
            alt="SplitMonies"
          />
        ...
        <div className="mt-8 max-w-3xl text-xl mx-auto text-center font-semibold text-gray-400">
          Building SplitMonies has been a highly enjoyable collaboration between
          Karma and numerous Nouners. We are continually seeking ways to enhance
          SplitMonies, so your suggestions for improvement are always welcome.
        </div>
        ...
        >
          {navigation.main.map((item) => (
            <div key={item.name}>
              <a
                href={item.href}
                className="text-xl leading-6 text-gray-600 hover:text-gray-900"
              >
                {item.name}
              </a>
            </div>
        ...
        <div className="mt-8 flex justify-center space-x-10">
          {navigation.social.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="text-gray-400 hover:text-gray-200 duration-200 transition-all ease-in-out"
              target="_blank"
              passHref={true}
            >
